...
export class ProgramOverviewEffects {
  ...
  @Effect() loadPrograms$ = this.actions$
    ...
    .flatMap(() => this.data.loadPrograms())
  ...
  @Effect() updateProgram$ = this.actions$
    ...
    .flatMap(program => this.data.updateProgram(program))
  ...
  @Effect() createProgram$ = this.actions$
    ...
    .map(action => action.payload)
    .flatMap(program => this.data.createProgram(program))
  ...
  @Effect() deleteProgram$ = this.actions$
    ...
    .map(action => action.payload)
    .flatMap(program => this.data.deleteProgram(program))
